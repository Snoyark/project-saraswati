...
Feed into Chroma
*/
const init = async function(topic: string) {
...
const run = async function(topic: string, vectorstore: Chroma) {
  ...
  _.forEach(search_results, async result => {
    // Download the pdf
    const file_path = `./${gen_utils.getNanoSecTime}.pdf`
    await dl_utils.downloadPdf(result.pdf_link, file_path)
    ...
    await vectorstore.addDocuments(docs, { ids: [result.title, `${result.title}.summary`] })
...
const main = async function(topic: string) {
  const collection = await init(topic)
  while (true) {
    console.log('started run')
    await run(topic, collection)
